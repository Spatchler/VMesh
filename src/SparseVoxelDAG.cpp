...
VMesh::SparseVoxelDAG::SparseVoxelDAG(uint pSize)
...
void VMesh::SparseVoxelDAG::insert(const glm::vec3& pPoint, const VoxelData& pData) {
...
uint VMesh::SparseVoxelDAG::getSize() {
  return mSize;
}

uint VMesh::SparseVoxelDAG::getMidpoint() {
  return mMidpoint;
}

void VMesh::SparseVoxelDAG::print() {
  std::println("\n------------------------------------");
  std::println("Indices:");
...
uint VMesh::SparseVoxelDAG::toChildIndex(glm::vec3 pPos) {
...
glm::vec3 VMesh::SparseVoxelDAG::toPos(uint pChildIndex) {
  glm::vec3 pos;
  pos.x = 1 & pChildIndex;
...
void VMesh::SparseVoxelDAG::insertImpl(const glm::vec3& pPoint, const uint& pDataIndex, uint pNodeIndex, uint pNodeSize, glm::vec3 pNodeOrigin) {
